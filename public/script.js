...
cartSummary.innerHTML = `
<p>Total: £${total.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
...
async function proceedToCheckout() {
    try {
        ...
        for (let [item, count] of Object.entries(itemCounts)) {
            items.push({
                id: itemIds[item], // Send the item ID
                quantity: count,
            });
        }

        console.log("Items to be sent to server:", items); // Debugging
        ...
        const response = await fetch('/create-checkout-session', {
            ...
            body: JSON.stringify({ items }), // Send correct structure
        });
        ...
        const { id } = await response.json(); // Extract the session ID directly

        // Initialize Stripe object with your publishable key
        const stripe = Stripe('pk_live_51PqLhOFDR0ud356M7EymBKLS9Lwr1DCA8YZDKyc3LrxvfmilhSJiO4re9oYy3INOmgRaEhQBJvIuJ5DG6Zpcl0Gx00W1GMJLua');
